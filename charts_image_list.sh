...
#get the image names from values file
#sleep .03
if [[ $f = "elasticsearch-curator" || $f = "couchdb" || $f = "azuremonitor-containers" || $f = "vault-operator" || $f = "telegraf" || $f = "sumologic-fluentd" || $f = "sumokube" || $f = "stolon" || $f = "spinnaker" || $f = "sonatype-nexus" || $f = "signalfx-agent" || $f = "sentry" || $f = "satisfy" || $f = "rethinkdb" || $f = "reloader" || $f = "prometheus-mysql-exporter" || $f = "msoms" || $f = "mongodb-replicaset" || $f = "artifactory" || $f = "artifactory-ha" || $f = "mattermost-team-edition" || $f = "external-dns" || $f = "gitlab-ce" || $f = "gitlab-ee" || $f = "good" || $f = "influxdb" || $f = "ingressmonitorcontroller" || $f = "ipfs" || $f = "external-dns"  ]]
then
 echo "go get $f - is currently excluded due to bad values files ########";
else
#for manual debugging of chart to image match up
...
echo "go get $f from invubator helm"

grep -i "repo:\|repository:\|image:\|Image:" $incubatorcharts/$f/values.yaml | grep -v "sampleEhcacheClient" |  grep -v "sysctlImage:" | grep -v "hyperkubeImage:" | grep -v "prometheusConfigReloader" | grep -v "configmapReloadImage:" |grep -v "mcImage:" | grep -v "waitImage:" | grep -v "AlwaysPullfalse:" | grep -v "downloadDashboards" | grep -v "chownDataImage:" | grep -v "gocd/gocd-" | grep -v "#" | grep -v "busyboxImage" | grep -v "initImage:" | sed -e "s/repository: //g" |  sed -e "s/repo: //g" | sed -e 's/RequestCertsImage://g' | sed -e "s/image://g" | sed -e 's/Image://g' | sed -e "s/\"//g" | sed -e "s/ //g" | grep -v ":" >> image.notag.list

grep -i "tag:" $incubatorcharts/$f/values.yaml | grep -v "#" |  grep -v "imageTag: \"9.6.2\"" | grep -v "kubeTag:" | sed -e "s/ImageTag://g" |  sed -e "s/RequestCertsImageTag://g" | sed -e "s/host.*//g" | sed  -e "s/kubeTag:kube//g" | sed -e 's/testImageTag:master//g' | sed -e "s/tag://g" | sed -e "s/ //g" | sed -e "s/imageTag://g" | sed -e "s/\'//g" | sed -e "s/\"//g" >> tag.noimage.list
grep -i "initContainerImage: " $incubatorcharts/$f/values.yaml | grep -v "#" | sed -e "s/\"//g" | sed -e "s/initContainerImage://g" | sed -e "s/ //g" | grep -v ":" >> image.notag.list
grep -i "image:" $incubatorcharts/$f/values.yaml | grep -v "#" | sed -e "s/\"//g" | sed -e "s/image://g" | sed -e "s/ImageTag://g" | sed -e "s/RequestCertsImageTag://g" | sed -e "s/ //g" | grep ":"  >> image.list
fi
if [[ $f = "distribution" ]]
then
grep -i "tag:" $incubatorcharts/$f/values.yaml | grep -v "#" | sed -e "s/ImageTag://g" |  sed -e "s/RequestCertsImageTag://g" | sed -e "s/host.*//g" | sed  -e "s/kubeTag:kube//g" | sed -e 's/testImageTag:master//g' | sed -e "s/tag://g" | sed -e "s/ //g" | sed -e "s/imageTag://g" | sed -e "s/\'//g" | sed -e "s/\"//g" >> tag.noimage.list
fi
if [[ $f = "mission-control" ]]
then
grep -i "tag:" $incubatorcharts/$f/values.yaml | grep -v "3.6.3-0" | grep -v "#" | sed -e "s/ImageTag://g" |  sed -e "s/RequestCertsImageTag://g" | sed -e "s/host.*//g" | sed  -e "s/kubeTag:kube//g" | sed -e 's/testImageTag:master//g' | sed -e "s/tag://g" | sed -e "s/ //g" | sed -e "s/imageTag://g" | sed -e "s/\'//g" | sed -e "s/\"//g" >> tag.noimage.list
grep -i "tag:" $incubatorcharts/$f/values.yaml | grep -v "3.6.3-0" | grep -v "#" | sed -e "s/ImageTag://g" |  sed -e "s/RequestCertsImageTag://g" | sed -e "s/host.*//g" | sed  -e "s/kubeTag:kube//g" | sed -e 's/testImageTag:master//g' | sed -e "s/tag://g" | sed -e "s/ //g" | sed -e "s/imageTag://g" | sed -e "s/\'//g" | sed -e "s/\"//g" >> tag.noimage.list
grep -i "tag:" $incubatorcharts/$f/values.yaml | grep -v "3.6.3-0" | grep -v "#" | sed -e "s/ImageTag://g" |  sed -e "s/RequestCertsImageTag://g" | sed -e "s/host.*//g" | sed  -e "s/kubeTag:kube//g" | sed -e 's/testImageTag:master//g' | sed -e "s/tag://g" | sed -e "s/ //g" | sed -e "s/imageTag://g" | sed -e "s/\'//g" | sed -e "s/\"//g" >> tag.noimage.list
...
if [[ $f = "spring-cloud-data-flow" ]]
then
grep -i "version:" $incubatorcharts/$f/values.yaml | grep "version:" | sed -e "s/host.*//g" | sed  -e "s/kubeTag:kube//g" | sed -e 's/testImageTag:master//g' | sed -e "s/tag://g" | sed -e "s/ //g" | sed -e "s/imageTag://g" | sed -e "s/\'//g" | sed -e "s/\"//g" >> tag.noimage.list
fi
